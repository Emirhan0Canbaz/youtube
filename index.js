...
.setThumbnail()
.addField(`c bot | Teşekkürler`, `**Selamlar, Ben Emirhan (cod3j botun  ' Geliştiricisi) Öncelikle Botumuzu Eklediğiniz ve Bize Destek Olduğunuz İçin Sizlere Teşekkürlerimi Sunarım**`)
.addField(`Alone | Prefix`, `**cod3j Botun Prefixi(ön eki) = \`${fynx.prefix}\`\n\n Değiştirebilmek için \`${fynx.prefix}prefix\` Yazabilirsiniz.**`)
.addField(`cod3j bot | Nasıl Kullanılır?`, `**cod3j bot botun tüm özelliklerinden yararlanabilmek için sadece \`${fynx.prefix}yardım\` yazmanız yeterlidir.**`)
.addField(`cod3j bot | Linkler`|"https://discord.gg/tf2cNTfjqz"| `**Bot komut veya sohbet Kanalına -davet Yazmanız Yeterlidir**`)
.setFooter(`cod3j Yazılım | Gelişmiş Türkçe Bot | iletişim cod3j#6371 | İg : Emirhan_exp |2022`)
.setTimestamp();
...
.setDescription(`<:a_:821738957997211659>  <@!${message.author.id}> **adlı kullanıcı tarafından** <#${message.channel.id}> **kanalına gönderilen mesaj silindi!** \n\nSilinen Mesaj: **${message.content}**`)
  .setFooter("Cedric Bot | Log Sistemi")
...
if (!modlogkanal) return;
    if (channel.type === "text") {
                ...
                .setFooter(` Cedric Bot | Log Sistemi Kanal ID: ${channel.id}`)
                modlogkanal.send({embed});
            };
...
client.on("message", async message => {
  ...
  const lus = await db.fetch(`Reklam Engel _${message.guild.id}`)
  if (lus) {
    ...
    if (reklamengel.some(word => message.content.toLowerCase().includes(word))) {
      try {
        if (!message.member.permissions.has('BAN_MEMBERS')) {
          ...
          return message.channel.send(`<:a_:821738957997211659>  **Hey ${message.author} Dur! Bu Sunucuda Reklamı ve küfürü  Engelliyorum!**`).then(message => message.delete(3000));
...
client.on('ready', () => {
  client.channels.cache.get('1030827251815686224').join();
});